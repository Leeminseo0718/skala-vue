 ...
 * 날씨 대시보드
 *
 * 종합과제 본체. 매일 배운 내용을 이 파일 하나에 계속 얹어 가며 키운다.
 *
 * [1단계 · 1일차] Vue 문법
 *   - v-for 배열 렌더링 (:key 에 id 바인딩)
 *   - v-if / v-else 조건부 렌더링 (25도 기준)
 *   - :value + @input 양방향 바인딩 (한글 IME 처리)
 *   - 이벤트 수식어 (.stop 으로 버블링 차단, .enter 키 수식어)
 *
 * [2단계 · 2일차] Composition API
      ...
      <ul v-else class="card-list">
        ...
        >
          ...
          <div class="card-bottom">
            ...
            <!-- .stop 으로 부모 카드로의 이벤트 버블링 차단 -->
            <button class="btn-detail" @click.stop="showDetail(city)">
